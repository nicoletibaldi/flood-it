...
Display.prototype.setupBoard = function () {
  ...
  for (var i = 0; i < Math.pow(this.game.size, 2); i++) {
      var randColor = this.colors[Math.floor(Math.random() * Constants.COLORS.length)];
      var $li;
      if (i === 0) {
...
Display.prototype.handleSize = function ($li) {
  if (this.game.size === Constants.EASY_GRID_SIZE) {
    $li.addClass("easy-square");
  } else if (this.game.size === Constants.MEDIUM_GRID_SIZE) {
    $li.addClass("med-square");
  } else if (this.game.size === Constants.HARD_GRID_SIZE){
...
// Find the corresponding numbers for top, bottom, left, right neighbors of a given square
...
// Find the adjacent squares (using adjSquares function) of 'flooded'squares,
// and create an array
...
// Determine if there are any squares to flood - this is needed as a base
// case for our recursive function
...
// Recursive solution that will 'flood' the proper squares with the user's
// selected color
